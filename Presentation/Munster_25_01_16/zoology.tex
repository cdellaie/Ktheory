...
The Baum-Connes conjecture has been proven for a very large class of groups: almost connected locally compact Hausdorff groups, a-T-menable groups, hyperbolic groups, etc. The main idea behind the vast majority of these proofs is the \textit{Dirac-Dual-Dirac} method:
\begin{itemize}
...
We will give now the construction while going through some reminders.\\

Let $(X,d)$ be a Hilbert-Hadamard space. We will denote by $\R_t$ the real vector space $\R$ for positive $t$, $0$ otherwise. Let $T= X\times \R_+$, and for each $(x,t)\in T$, let $C_{x,t}$ be the Clifford algebra associated to the real vector space $T_x X \oplus \R_t$ endowed with the nondegenerate bilinear symmetric form $q(u_x,v)= \|u_x\|_x^2+ v^2$, and form the infinite $C^*$-algebraic product 
...
Let $\Gamma$ be the complex subspace generated by $\{a_{f,x_0} \}_{f\in C_0(\R) , x_0\in X}$.
...
Another description in term of $C_0(Z)$-algebras? Later. Also: is it a groupoid algebra with $G^0 = A(X)_0$\\
