...
\item[$\bullet$] Non exact $C^*$-algebra: to my knowledge only one example is known: the reduced $C^*$-algebra $C_r^*(\Gamma) $ of a finitely generated group whose Cayley graph contains expander. Using Ozawa's resulte \cite{Ozawa}, one can construct finite dimensional $C^*$-algebras $M_{X_n}$ such that 
