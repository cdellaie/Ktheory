...
We recall some facts about representation theory of $C^*$-algebras. We will denote $\phi : A \rightarrow B(H)$ and $\sigma : A \rightarrow B(K)$ two representations, i.e. $*$-homomorphisms. Then one can define the space of intertwiners
...
whose image we denote by $H^\sigma$ or $H^K$. It is the subspace of $\sigma$-\textit{isotypical components} of $H$. Then $H_\sigma$ will denote the orthogonal complement of $H^\sigma$. If $\sigma$ and $\sigma'$ are two non-equivalent irreducible representations, then $H^\sigma$ and $H^{\sigma'}$ are in direct sum, so that if $H$ is finite dimensional, only finitely many $H^\sigma$ are non zero.\\

Fix a finite set $F= \{a_i\}_{i=1,N}$ in $A$ and $\Delta_{\sigma, \phi} = \sum_{a\in F} x_a^* x_a $ with $x_a =  \phi(a) \otimes 1 - 1 \otimes \sigma(a)$. Then
\[Ker (\Delta_{\sigma, \phi}) \cong Hom_{C^*(F)}(K,H).\] 

Let $A\subset B(H)$ be a finitely generated $C^\ast$-algebra, and $l(a)$ the associated length. Suppose
\begin{itemize} 
...
Define $L^2(X,H)$ to be the completion of the sections $ \Gamma_c(G,r^* H)$ with the $C_0(G^0)$-valued inner product
...
and $C_0(G)$-module structure given by $(\eta f)(x, g) = \eta(x,g)f(g)$.\\ 
The same is true when $r$ replaces $s$, and the map $(U\eta)(x,g) = \eta(xg,g)$ defines a unitary 
\[U : s^* L^2(X,H) \rightarrow r^* L^2(X,H)\]
such that $U_g U_h = U_{gh}$ for all $(g,h)\in G^2$.\\
...
\begin{cor}
The isomorphism $V$ induces an ismorphism
...
The question is to see how far we can push that technique, here deducing the conjecture for $SL(2,\Z)$ from $\Z$.\\
