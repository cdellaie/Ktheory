...
Recall that a discrete metric space has (geometric) property (T) if there exists a Kazdhan pair $(E,\varepsilon)$ such that, for every $*$-representation 
\[\pi : \C_u(X) \rightarrow B(H)\]
and every non zero vector $\xi \in H_\pi$, there exists $a\in \C_E[X]_{+,1}$ such that 
\[\| a\xi -\Psi(a)\xi \| > \varepsilon \|\xi\|.\]

A coarse space is almost monogenic if there exists an entourage $E\in \mathcal E_X$ such that for all other entourage $F$ there is a $n$ such that $F \backslash E^n $ is finite. Coarse disjoint unions of finite graphs with uniformly bounded degree are almost monogenic. For such a space, $E$ can be decomposed as 
\[E= \coprod_{s\in S} graph(s)\]
for a finite symmetric set $S\subset S_X$. Then, finite propagation operators are a $*$-algebra generated by $S$ and the finite rank operators on $\ell^2(X)$. Define 
\[\Delta_0 = \sum_{s\in S} (s-s^*s)^* (s-s^*s),\]
and notice that for every $F\in \mathcal E_X$, almost monogenicity ensures that 
\[\Delta_0 - \Delta_F\]
is of finite rank.

\begin{lem}
The maximal spectrum of $\Delta_0$ and $\Delta_F$ coincide. As a consequence, $X$ has property (T) iff $sp_m(\Delta_0)\subset \{0\}\cup (c, \infty)$.  
\end{lem}

In particular, if $X$ has (T), then $X$ is an expander, which generalizes (a little) a result of Willett-Yu.

\begin{proof}
We use the fact that the spectrum is invariant under inclusions of $C^*$-algebras.\\

Let $\omega$ be a nonprincipal ultrafilter. The $S$-invariant state $\lim_\omega tr_{X_n}: \prod M_{X_n} \rightarrow \C$ kills the compact operators, and thus defines a $S$-invariant CP map that extends to
\[C^*_m(X)/\mathfrak K(\ell^2 X)\rightarrow C^*(S)\]
which is a section of the quotient map: $C^*(S)$ embeds in $C^*_m(X)/\mathfrak K(\ell^2 X)$ and the spectrum of $\Delta_0$ is the same in both these algebras, and $\Delta_0 = \Delta_F \ mod \ \mathfrak K(\ell^2X)$.
\end{proof}

\begin{thm}
Let $X$ be a coarse disjoint union If $X$ has property (T), then $C_u^*(X)$ and $C^*_m(X)$ are not $K$-exact.
\end{thm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Number of irreducible representations}

Maybe I should point out that geometric property (T) for a coarse disjoint union of finite graphs whose cardinality diverges to $\infty$ implies that the space is an expander, which is a slight improvement of Willett-Yu. The question is: is expansion equivalent to a property similiar to $(\tau )$?

Spakula breaks down the proof into different steps. 
\begin{itemize}
\item[$\bullet$] there are irreps $H_n<\ell^2(X_n)$ for each $n$ such that $dim H_N$ goes to $\infty$,
\item[$\bullet$] the strengthening of property tau he is using implies that the Markov operator has a spectral gap at 1,
\item[$\bullet$] the corresponding spectral projection $p$ goes to 0 in the quotient $C^*_u(X)\otimes \prod /\oplus B_M$. This uses that there are only finitely many M's such that $K_M<H_N$.
\item[$\bullet$] the class $[p]$ cannot come from $C^*_u(X)\otimes \oplus B_m$. This uses an adaptation of Higson's trace argument: there is a $*$-homomorphism
\[C^*_u(X)\otimes \prod B_m \rightarrow \prod / \oplus B_m\]
which kills $C^* \otimes \oplus $, and the image of $p$ is not zero even in $K$-theory. This generalizes easily: if $X$ is an expander, $C^*_u(X)$ embeds into the direct limit of the 
\[A_n =B(\bigoplus_{0\leq k \leq n} \ell^2(X_k)) \oplus \prod_{k>n}B(\ell^2(X_k)).\]
If $\omega$ is a principal ultrafilter on $\N$, then $\lim_\omega \tau_n(a_{nn})$ is well defined on $C^*\otimes B$ and kills $C^*\otimes J$. The trace of $p_{nn}$ will never be zero because of the first point.
\end{itemize}

When $\sigma$ is f.d., the whole of $hom_A(\sigma, \pi)$ can be identified to the Hilbert Schmidt operators $\pi \otimes_2 \sigma^*$, with $\| T\|_2^2 = tr(T^*T)$. Under that identification, an intertwiner $T=\sum_i x_i \otimes y_i$ satisfies
\[ \|T\| = \sup \{ \ \|\sum x_i a y_i^* \|_2\ : \ \|a\|_2\leq 1 \}\]

In the presence of spectral gap, there are only finitely many irreps of dimensions $n$ for a $C^*$-algebra $A$ that is finitely generated by a set of partial isometries. (This is an adaptation of the argument 3.4 of Lubotzky,Zuk, On property Tau).

Indeed, say $A=C^*S$ and define the map
\[x: \sigma \mapsto (\sigma(s))_{s\in S} \in S(\C^d).\]
This means that a representation of $A$ is determinded by the image of the generators. Being partial isometries, their HS norm is the rank of their domain projection. The image of $x$ is thus contained in the union of spheres of radius $\sqrt{d}, 2\sqrt{d},..., \sqrt{d|S|}$.

Now, 
\[\begin{split}
\| \rho_s\otimes \sigma_s -I \| & = \sup \{\  \| \rho_s x \sigma_s^* -x\|_2 \ : \ \|x\|_2\leq 1\} \\
									& =  \sup \{\  \| \rho_s x  -x \sigma_s\|_2 \ : \ \|x\|_2\leq 1\} \\
									& \geq \|\rho_s -\sigma_s \| \\
									\end{split}\]
...
Other examples:
\begin{itemize}
\item[$\bullet$] Let $\Lambda < G$ be finitely generated, relatively hyperbolic. Choose a generating set $B$ for $\Lambda$ and $S=B\cup R$ for $G$. The letters stand for \textit{blue} and \textit{red}. The Cayley graph of $G= \coprod g\Lambda$ can be seen as containing copies of $Cay(\Lambda, B)$, connected by red vertices. The relative hyperbolicity is, if I remember correctly, equivalent to the hyperbolicity of the graph obtained from this one by connecting each copy of the Cayley graph of $\Lambda$ to a point at infinity (the \textit{coning point}). This vertex has stabilizer $g\Lambda g^{-1}$. This is a non locally finite space that is called the \textit{coning} of the Cayley graph of $G$.
\item[$\bullet$] let $\Lambda $ be the upper triangular matrices in $G=SL(2,\Z)$. Then if $X$ is the associated coning, the stabilizers are isomorphic to $\Z$. That's the example of the introduction.
\item[$\bullet$] Let $M$ be the complement of the figure eight knot (more generally any hyperbolic manifold with one total cusp), and $G=\pi_1(M)$. The stabilizer is $\Z\oplus \Z$.
\item[$\bullet$] Graph of pants of a complexity-2 surface. For hyperbolic 3-manifolds, the Teichm\"{u}ller space with the Weil-Petersson metric is quasi-isometric to the pants graph of $\Sigma$ (usually not hyperbolic as it will contain a copy of the Euclidean plane, but relatively hyperbolic?).
\end{itemize}
...
\subsubsection{Green's isomorphism}
...
The nice trick used in the paper on extensions is the following: given a surjective group morphism $\phi: G \rightarrow Q$, one considers the action of $G\times Q$ on $Q$ given by 
...
What properties do we know about $SL(n,\Z)$?\\
...
%Here the action is restricted to $X\times X$ so that it is transitive, and $x\in X \subset \beta X$. 
...
First, if $\Lambda= \phi^{-1}_\omega(K)$, let us show that $\Lambda$ contains $G_\omega$ a subgroup of finite index. Let $W$ be a neighborhood of $w$, small enough that we can find a  finite cover of $K_W$ by bisections $U_i$, $i=1,N$, such that $W\subset U_i$ for every $i=1,N$. Let $\alpha_i $ be the corresponding partial homeomorphisms. Then 
...
Note that a $C(G^0)$-algebra $A$ defines a \textit{continuous field of $C^*$-algebra} $\mathcal A$ ( see \cite{}, Dixmier section ...), with fibres
