...
If $E$ is a Hibert-module, $E$ is full if $\langle \psi,\eta\rangle$ generates $A$. If $A$ is filtered, the natural filtration on $K(E)$ is ?

\begin{prop}
If $A$ is a filtered $C^*$-algebra and $\mathcal E$ a full Hilbert-module over $A$, there are controlled isomorphisms
\[M_{\mathcal E}: \hat K(\mathcal E) \rightarrow \hat K(A)\]
where $K(\mathcal E)$ is equiped with the natural filtration. We call $M_{\mathcal E}$ the Morita equivalence.
\end{prop}
