...
Remarks on generating sets. Example for a finitely generated groups and their box spaces when r.f. In that case
\[E =\cup_{s\in S} graph(s)\]
is a generating set. Here $s\in S$ is identified with the gloabally defined partial translation $X\rightarrow X ; x\mapsto s\cdot x $. \\

What about a coarse disjoint union of finite graphs? If $d(X_i,X_j)=\infty$ when $i\neq j$, then we can do the same. (details) 
...
\[\Delta^Ef(x) = f(x) -\frac{1}{|E_x|} \sum_{y\in E_x} f(y). \]
If $E$ is symmetric and $E = \coprod graph(s)$ with $dom(s)=X$, then 
\[\delta^E = \frac{1}{2|S|}\sum_{s\in S} (s^*s-s)^*(s^*s-s).\]
\begin{prop}
\begin{itemize}
\item[$\bullet$] $ker \Delta = H^\pi $
\item[$\bullet$] $X$ has (T) iff there exists $c>0$, for every $\pi: \C_u[X] \rightarrow B(H)$, 
