...
\textbf{Geometrical} &  
		...
		\exists \alpha : G \rightarrow Aff(H)  \\ 
			...
			& \frac{|F\Delta s F|}{|F|}\leq \varepsilon & \\
\hline
\textbf{Kernels} &
		\forall \varepsilon, r>0, &
...
\textbf{Hulanicki-Reiter} &
		...
		&  \|s\cdot f -f\|_2< \varepsilon , \forall |s|\leq r & \\
\hline
\end{array}\]
\end{table} 
...
\textbf{Geometrical} &  
		...
		\exists f: X \rightarrow H   \\ 
			...
			& \frac{|A_x\Delta A_y|}{|A_x\cap A_y|}\leq \varepsilon \forall (x,y)\in \Delta_r & \\
\hline
\textbf{Kernels} &
		\forall \varepsilon, r>0, &
		\exists \phi: X\times X\rightarrow \R \text{ s.t. } \\
			&\exists \phi: X\times X\rightarrow \R \text{ PT s.t. } & \text{ CNT and proper} \\
			&supp(\phi)\subset \Delta_s \& \|1-\phi\|_{\infty,\Delta_r}\leq \varepsilon  & \\
\hline
\textbf{Higson-Roe} &
		\forall \varepsilon, r>0, &
		\\
