...
\begin{prop}[\cite{PodlesWo}, Theorem $2.1$] There exists a unique $*$-homomorphism $\hat \Delta : c_0(\hat{\mathbb G}) \rightarrow \mathcal M(c_0(\hat{\mathbb G})\otimes c_0(\hat{\mathbb G}))$ such that \[(\hat\Delta\otimes id ) U = U_{12}U_{13}.\]
...
Notice that every $KK^{\hat{\mathbb G}}$-element satisfies property $(d)$ for some universal $d>0$ independent of $\mathbb G$ as well. Indeed, the proof of \cite{LaffOY} works without modification in the setting of discrete quantum groups.
