...
Let $G$ be an étale groupoid and let $\mathcal E$ be the set of its compact subsets. In this section, we define controlled $K$-theory groups for $G$. By analogy with the terminology used in Coarse Geometry, we will call $E\in \mathcal{E}$ a controlled subset. Notice that if $E,E'\in \mathcal E$, then $E E'\in\mathcal E$, where $EE' := \{ gg' , g\in E,g'\in E'\}$.\\
...
If $A$ is a non-unital filtered $C^*$-algebra, we will by default endowed its unitalization $\tilde A$ with the filtration $\tilde A_E = A_E + \C$.\\
...
One has also forgetful inclusions $\iota_{\varepsilon,E} : K_*^{\varepsilon,E}\rightarrow K_*(A)$ given by $[p,l]_{\varepsilon,E} \mapsto [\kappa_0(p)]-[1_l]$ and $[u]_{\varepsilon,E} \mapsto [\kappa_0(u)] $, and 
...
We can also compare control pairs. Indeed, define the following partial order : $(\alpha,h) \leq (\beta,k)$ if $\alpha \leq \beta$ and $h_\varepsilon\leq k_\varepsilon$ for all $\varepsilon\in (0,\frac{1}{4\alpha})$.\\ 
...
\begin{thm}
There exists a control pair $(\lambda,h)$ such that for any completely filtered extension of $C^*$-algebras 
