...
We will say that $\alpha_{A,B}$ is quantitatively injective if there exists $\lambda \geq 1$ such that, for every $\varepsilon\in (0,\frac{1}{4\alpha_\tau})$ and every $F\in\mathcal E$, there exists $F'\in\mathcal E$ such that $F\subseteq F'$ and the following holds : 
...
We will say that $\alpha_{A,B}$ is quantitatively surjective if there exists $\lambda \geq 1$ such that, for every $\varepsilon \in (0,\frac{1}{4\alpha_\tau})$, $F\in\mathcal E$, there exist $F'\in\mathcal E$ such that $F\subseteq F'$ and the following holds 
\[ \forall y\in K^{\varepsilon,F}(A\otimes B), \exists x\in K^{\varepsilon, F'}(A)\otimes K(B) \text{ such that }
\iota_{\alpha_\tau\varepsilon,h_{\tau,\varepsilon}. F}^{\lambda\varepsilon,F'}\circ\alpha^{\varepsilon,F'}_{A,B}(x)=\iota_{\varepsilon,F}^{\lambda\varepsilon,F'}(y).\] 


\begin{definition}
A filtered $C^*$-algebra $(A,\mathcal E)$ is said to satisfy the quantitative Künneth formula if, for every $C^*$-algebra $B$ such that $K_*(B)$ is a free abelian group, $\hat\alpha_{A,B}$ is quantitatively injective and quantitatively surjective. 
\end{definition}
...
Set $m = \sup (\alpha_\tau,\lambda)$, hence $\iota_{\varepsilon,F}^{m\varepsilon,h_\varepsilon F''}(y)=\iota_{\alpha_\tau\varepsilon,h_\varepsilon F''}^{m\varepsilon,h_\varepsilon F''}\circ\alpha_{A\rtimes_r G,B}^{\varepsilon,F''} (z_2)$ and $\hat \alpha_{A\rtimes_r G,B} $ is quantitatively surjective.\\
