...
\begin{rk}
For free groupoids, the third condition in theorem \ref{Kunneth}, "$\alpha_{Res_H^G(A),B}^{H,V}$ is an isomorphism", reduces to "$A$ satisfies the Kunneth formula". Indeed, the only non trivial subgroupoid is the trivial one.
\end{rk}
