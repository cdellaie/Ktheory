...
\item[$\bullet$] unit and inverse map $e : G^{(0)}\rightarrow G; x\mapsto e_x$ and $\text{inv} : G\rightarrow G: g\mapsto g^{-1}$ which are continous and such that $\forall g\in G, gg^{-1}=e_{s(g)} \text{ and }g^{-1}g=e_{r(g)}$, and $g e_{s(g)} = e_{r(g)} g =g$.
\end{itemize} 
\end{definition}
