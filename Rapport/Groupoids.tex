...
It is naturally an involutive algebra with the convolution product
...
\end{definition}
 % A FINIR
\begin{Expl} If $B$ is a $G$-algebra, define $l^2(G,B)$ as the completion of $C_c(G,B)$ with respect to the $B$-valued inner product : 
\[\langle \xi ,\eta\rangle (x) = \sum_{g\in G_x} \xi(g)^*\eta(g) \in B_x.\]
We made the following confusion : $\langle \xi ,\eta\rangle : x\mapsto \langle \xi ,\eta\rangle (x)$ defines an element of $C($ .
\end{Expl}
