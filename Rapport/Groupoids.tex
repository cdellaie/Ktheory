...
\begin{Expl}[Equivalence relations] More generally, consider an equivalence relation $R \subset X\times X$ on a topological space $X$. It naturally has a structure of topological groupoid, as a subgroupoid of $X\times X$. 
\end{Expl}
...
Following \cite{TuBC2}, we present now an important class of $G$-spaces called $G$-simplicial complexes.
...
\end{itemize}
For $\eta\in \Delta$, $\text{supp }\eta$ is called a simplex.
\end{itemize}
...
\item[$\bullet$] $Z_j \setminus Z_{j-1}$ is $G$-equivariantly homeomorphic to $\mathring\sigma_j \times ?$, where $\mathring\sigma_j $ is the interior of the standard simplex of dimension $j$.
\end{itemize}
