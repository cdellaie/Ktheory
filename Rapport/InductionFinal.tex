...
\begin{dem}
Let us show that $\tilde\pi(a). (\tilde T^2-1)$ and $\tilde\pi(a). (\tilde T^2-1)$ are compact operators, for any $a\in C_0(G\times_H V)$. Recall that, for every Hilbert $B$-module $E$ and any $\xi,\eta\in E$, $\theta_{\xi,\eta}\in\mathfrak K_B(E)$ denotes the rank-one operator $x\mapsto \xi \langle \eta,x\rangle$. For $\theta_{\xi,\eta}$, we will denote by $T_{\xi,\eta}$ the $H$-invariant operator obtained by averaging over the compact subgroupoid $H$. The operators $\tilde\pi(a). (\tilde T^2-1)$ and $\tilde\pi(a). (\tilde T^2-1)$ are in the closure of the algebra generated by the $T_{\xi, \eta}$. Hence, it is sufficient to prove that, for any $a\in C_0(G\times_H V)$, $\tilde \pi(a) T_{\xi,\eta} \in \mathfrak K_B(\tilde E)$.\\ 

Let $\{G_i\}_i$ be a cover of $G_{|H^{(0)}}$ by compact open bisections. For each $i$, let $\{H_{ij}\}_j$ be an open cover of $r^{-1}( s(G_i))\cap H$ by compact open bisections $H_{ij} \subseteq H$. Up to taking a subcover, we can suppose $s(G_i) = r(H_{ij})$, for every $i$ and $j$. Put $\tilde G_i = \coprod G_i \circ H_{ij}$, which is a $H$-invariant open subset of $G$, when taking right-translation of $H$ on $G$. Let $\{\phi_i\}_i$ be continuous functions $\phi_i : G^{(0)}\rightarrow [0,1]$ such that 
\[\sum_{i} \phi_i(x) = 1\quad \forall x\in r(G_{|H^{(0)}}) \quad \text{ and } \quad supp \ \phi_i\subseteq r(G_i) .\] 
It is sufficient to prove that $\tilde \pi(\phi_i) T_{\xi,\eta} \in \mathfrak K_B(\tilde E)$.\\

For $x$ in $H^{(0)}$, let $g_x$ the only element of $G_i$ such that $s(g_x)= x$. If $\xi \in E$, define $\tilde \xi\in \tilde E$ the $H$-invariant element obtained from $\phi \otimes_s \xi \in C_0(\tilde G_i)\otimes_s E$. Then, if $x\in \tilde E$: 
\[\begin{array}{rcl}
\theta_{\tilde \xi,\tilde \eta}(x) 	& = & \tilde \xi \frac{1}{|H^x|} \sum_{g\in G^x} \alpha_g (\langle \tilde\eta (g), x(g) \rangle) \\
					& = & \tilde \xi \frac{1}{|H^x|} \sum_{h\in G_i^{s(g_x)} } \alpha_{g_x h} (\langle \tilde\eta (g_x h), x(g_x h) \rangle) \\
					& = & \tilde \xi  \alpha_{g_x} (\langle \eta, x(g_x) \rangle)  
\end{array}\]
Hence, $\theta_{\tilde \xi,\tilde \eta}(x)(g) = 0$ if $g\notin \tilde G_i$ et $\tilde \pi (\phi_i) \tilde T_{\xi,\eta} = \theta_{\tilde\xi,\tilde\eta}$, and the proof is complete.\\
\qed 
\end{dem}
