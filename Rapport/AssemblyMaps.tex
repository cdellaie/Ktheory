...
Let $X$ be a coarse space. The coarse homology group of $X$ with coefficients in an arbitrary $C^*$-algebra $B$ is defined as follows.
\[KX_*(X,B)=\varinjlim_{E\in \mathcal E_X} RK(P_E(X),B)\]
where the limit is taken along the inductive system $\{P_E(X)\}$. 
